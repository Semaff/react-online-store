...
const CategoryBrandModal = ({ show, setShow, view }) => {
  ...
  const unusedCategoryBrandsOptions = unusedCategoryBrands?.map(
    (brand) => brand.name + ` (Id: ${brand.id})`,
  );
  const usedCategoryBrandsOptions = category.brands?.map(
    (brand) => brand.name + ` (Id: ${brand.id})`,
  );
  ...
  useEffect(() => {
    if (Object.keys(category).length > 0) {
      const categoryBrandIds = category.brands.map(
        (categBrand) => categBrand.category_brand.brandId,
      );
