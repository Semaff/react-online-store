...
const TableRow = ({ id, img, name, price, salePrice, quantity, basket_product }) => {
  ...
  const handleChangeQuantity = (e) => {
    if (e.target.value < basket_product.quantity && e.target.value > 0) {
      dispatch(
        decrementProduct({ productId: id, quantity: basket_product.quantity - e.target.value }),
      );
    } else if (e.target.value > basket_product.quantity && e.target.value < quantity) {
      dispatch(
        incrementProduct({ productId: id, quantity: e.target.value - basket_product.quantity }),
      );
    }
  ...
  return (
    <tr>
      ...
      <td className="table__price">${total}</td>
    </tr>
  );
};
  ...
  return (
    <div className="table__responsive">
      ...
      <div className="table__btns">
        <input type="text" className="table__input" placeholder="Coupon code" ref={couponRef} />
